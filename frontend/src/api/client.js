...
export const predictTransaction = (t) => request("/predict", { method: "POST", body: JSON.stringify(t) });
export const getHistory         = (limit = 50, fraudOnly = false) => request(`/history?limit=${limit}&fraud_only=${fraudOnly}`);
export const getStats           = () => request("/stats");

export const batchPredict = (file, threshold = 0.5) => {
  const form = new FormData();
  form.append("file", file);
  return fetch(`${BASE_URL}/batch-predict?threshold=${threshold}`, { method: "POST", body: form })
    .then(async (res) => {
